# ============================================================================
#  prspct - free mint, one cell.  Paste the whole thing, put your key on the
#  line below, run.  It digs and claims until the mine is sold out.
#  Kaggle: Accelerator = GPU T4 x2, Internet = On.
# ============================================================================

PRIVATE_KEY = ""   # <-- your 64 character key here, in the quotes. Leave it
                   #     empty and the script asks for it in a hidden box
                   #     instead, which keeps it out of the saved notebook.

MARGIN = 192       # feet of extra difficulty, so a hash survives the mints
                   # that land before yours. The script raises it on a revert.
MINTS = 0          # 0: keep going until the mine is sold out

# ----------------------------------------------------------------------------
import subprocess, sys
print("installing cupy and the signer (about a minute the first time)...")
subprocess.run([sys.executable, "-m", "pip", "install", "-q",
                "cupy-cuda12x", "nvidia-cuda-nvrtc-cu12", "nvidia-cuda-runtime-cu12",
                "eth-account"], check=False)
print("done\n")
...
# ---------------------------------------------------------------- run it now
cuda_ready()      # give cupy a CUDA root before anything tries to compile
print("\nchecking the maths and the kernel before anything is signed...\n")
if self_test() != 0:
    raise SystemExit("self test failed - do not enter the key")

import os as _os
if PRIVATE_KEY.strip():
    _os.environ["PRSPCT_KEY"] = PRIVATE_KEY.strip()
elif not _os.environ.get("PRSPCT_KEY"):
    from getpass import getpass
    print("\nPRIVATE_KEY at the top of the cell is empty, so paste the key here.")
    print("ONLY the key - 64 characters - not the script.")
    _os.environ["PRSPCT_KEY"] = getpass("key (hidden): ").strip()

sys.argv = ["prspct", "auto", "--margin", str(MARGIN), "--mints", str(MINTS)]
try:
    main()
except KeyboardInterrupt:
    print("\nstopped")
